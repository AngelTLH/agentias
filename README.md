...
2. Abre el configurador. Tienes dos formas de hacerlo:
   * **Opción A (Doble clic - Recomendado)**: Haz doble clic en el archivo **`Iniciar_Agentias.bat`**.
     > [!IMPORTANT]
     > **Nota sobre la Alerta de Seguridad de Windows (SmartScreen)**:
     > Al descargar scripts (`.bat`) desde internet, Windows suele bloquearlos temporalmente. Si te aparece una advertencia de pantalla azul:
     > 1. Haz clic en **"Más información"** (More info).
     > 2. Selecciona **"Ejecutar de todos modos"** (Run anyway).
     > 
     > *Alternativamente, puedes hacer clic derecho sobre `Iniciar_Agentias.bat` -> **Propiedades** -> marcar la casilla **"Desbloquear"** (Unblock) abajo del todo -> **Aceptar**.*
   * **Opción B (Línea de comandos)**: Si prefieres no usar el `.bat` directamente, abre una terminal en la carpeta raíz y ejecuta:
     ```bash
     python Agentias_Vault/Dashboard/server.py
     ```
     Luego abre tu navegador y entra en `http://localhost:3000`.
3. Configura los parámetros de tu nuevo proyecto (nombre, ruta, agentes activos, tecnologías) y presiona **"Crear Proyecto"**.
4. Se generará la estructura completa con el archivo `prompt_inicio.txt` en la raíz del nuevo proyecto, listo para ser copiado y entregado a tu IA de preferencia (ChatGPT, Claude, Gemini, etc.).
